...
shaka.media.VideoWrapper.PlayheadMover = class {
  ...
  moveTo(timeInSeconds) {
    // It appears that this moveTo method doesn’t take into account that
    // timeInSeconds may be identical to the currentTime on the media element.
    //
    // This results in a tight loop of aborted requests as
    // mediaElement.currentTime is reset to the same value.
    // The tick timer only considers the move operation a success if
    // currentTime != originTime (current time at the start of the move op)
    if (timeInSeconds === this.mediaElement_.currentTime) {
      return;
    }
